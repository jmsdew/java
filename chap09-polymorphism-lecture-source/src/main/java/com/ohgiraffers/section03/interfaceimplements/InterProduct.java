...
public interface InterProduct extends Serializable {
    ...
    * 이유) 인터페이스는 생성자가 없기 때문에
    * */
    public static final int Max_NUM = 100;

    // 인터페이스는 상수 필드만 가질 수 있기 떄문에 모든 필드는 묵시적으로 public static final이 된다.
    int MIN_NUM = 10;


    // 인터페이스는 생성자를 가질 수 없다.
    // 인터페이스를 사용하는 목적은 추상 메소드를 묶어서 제공하기 위함이다.
    // 그렇기 때문에 INTERFACE를 생성하여 사용하는 일이 없어 어딘가에 상속하여 구현 후 사용하게 된다.
 //   public ImterProduct(){} -- 생성자 불가

    // 추상 메서드는 몸체를 갖지 않기 때문에 오류가 발생한다.
//    public abstract void nonStaticMethod(){}
    public abstract void nonStaticMethod();
    /*
    * 인터페이스 안에 작성한 메서드는 묵시적으로 public abstract 의 의미를 가진다 ( 다른 접근제한자 사용불가)
    * 따라서 인터페이스의 메소드를 오버라이딩 하는 경우
    * 반드시 접근 제한자를 public 으로 해야 오버라이딩이 가능하다.
    * */
    void abstMethod();
    
    // static 키워드를 이용하여 작성하는 경우 가능하다. (jdk 1.8에 추가된 기능)
    ...
    public default void defaultMethod(){
        System.out.println("interProduct 클래스의 default 메서드 호출됨");
    }
