...
public class Application {
    public static void main(String[] args){
        ...
        * */
        ...
        System.out.println();
        System.out.println("Rabbit인스턴스 생성 -----");
        ...
        Animal animal1 = new Rabbit();   //run은 있지만 비활성화  클래스 애니멀.
        Animal animal2 = new Tiger();
        
     //    Rabbit run1 =(Rabbit)new Animal();  //  rabbit 인스턴스 내용이 추가됨
        // 런타임 단계  -- 실제 프로그램 실행 단계
        System.out.println("동적 바인딩");
        animal1.cry();         // 호출할때 클래스 animal을 래빗형으로 전환해서 나오는 걸 동적 바인딩이라고 함.
        animal2.cry();
        System.out.println("클래스 타입의 형변환 확인 ----");
        ((Rabbit)animal1).jump();   /// animal 클래스에 없는 내용은 형변환을 해야 나타남. 동적 바인딩에 해당 안됨.
                                    /// 컴파일 단계  -- 프로그램 해석 단계 컴파일 이후 런타임
        ((Tiger)animal2).bite();

        System.out.println("형변환 오류");
        /*((Tiger)animal1).bite();                  // 컴파일러는 속고 런타임에서 오류가 남
        ((Rabbit)animal2).jump();                // animal 필드도 에러가 발생 */
        ...
        System.out.println("animal1이 Animal과 같은지 : " + (animal1 instanceof Animal));  //두가지를 가지는걸 다형성이라고 한다.
        System.out.println("animal 이 Rabbit과 같은가 : " +(animal1 instanceof Rabbit));
        System.out.println("animal이 Tiger와 같은가 : " + (animal1 instanceof Tiger));  
