...
public class Application {
    public static void main(String[] args){
        ...
        * 하지만 하위 버전의 호환성 때문에 한 번 에 제거되는 것은 아니고 남겨두었기 때문에 사용하는 것은 가능하다.
        *
        ...
        *    운초란? 협정 세계시에서 사용하는 세슘 원자 시계와 실제 지구의 자전/공전 속도를 기준으로 한 태양시의 차이로 인해
        *    발생한 오차를 보정하기 위해 추가하는 1초이다.
        *    12월 31일의 마지막에 추가하거나 혹은 6월 30일의 마지막에 추가된다.
        *    윤초는 사소해 보이지만 실제 2012년 링크드인과 같은 대규모 서비스의 서버를 마비시킨 버그를 발생한 적도 있다.
        *
        * 3. Calendar 클래스는 월을 나타내는 0부터 11까지 표현하는 불편함이 있다.
        *
        * jdk 1.8 에서는 이러한 모든 문제를 해결할 수 있는 time 패키지를 제공하고 있다.
        *
        *Date는 java.sql.Date와 java.util.Date가 존재한다.
        * 한 클래스에서 두 개의 타입을 전부 사용하게 되면 import 를 하더라도 사용하는 타입이
        * 어느 패키지에 있는지 모호성이 발생되어 import 를 하여도 클래스의 이름에 풀 패키지 경로를 입력해야 한다.
        * 주의하자.
        *
        * */
        ...
        /*getTime()은 1970년 1월 0시 0분 0초 이후 지난 시간을 milisecond로 계산하여 long 타입으로 반환한다.*/
        System.out.println(today.getTime());

        Date time = new Date(1698815745836L);
