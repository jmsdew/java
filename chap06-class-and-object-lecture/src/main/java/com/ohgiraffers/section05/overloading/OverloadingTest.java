...
public class OverloadingTest {
    ...
    public static void main(String[] args){
        ...
        * 시그니쳐라고 부른다. method(int num)이 부분이  시그니쳐이다.
        * 메소드의 시그니쳐가 달라야 하기 때문에 접근제한자나 반환형은 오버로딩 성립 요건애 해당하지 않는다.
        * */

        //1. 매개변수의 타입이나 갯수, 순서를 다르게 작성한다.
        //2. 같은 이름의 메소드를 사용
        //3. 시그니쳐부분이 달라야 한다.(int num)
    ...
    public char test( char gender){
        return gender;
    }
    public int test( int age){
        return age;
    }
    public int test(int weight, int tall){
        return weight + tall;
    }
    public boolean test(boolean inOut){
        return inOut;
    }
}
