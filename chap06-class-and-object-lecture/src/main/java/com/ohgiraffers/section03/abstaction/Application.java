...
public class Application {
    public static void main(String[] args){
        ...
        /*
        * 객체 지향 프로그래밍이란
        * 현실 세계의 모든 사건(event)는 객체의 상호작용에 의해 일어난다는 세계관을
        ...
        * */
        ...
        /*
        * 객체와 객체의 상호작용이란?
        * 객체와 객체는 메세지를 통해 서로 상호작용을 한다. 메세지를 보내는 쪽은 송신자라고 하고,
        ...
        * 카레이서 클래스
        *   1. 속성 : 자동차
        *      행위 ; 시동 걸기, 엑셀 밟기, 브레이크 밟기, 시동 끄기 (메소드)
        * 자동차 클래스
        *   1. 속성 : 시동의 상태, 속도
...
//        carRacer.startUp();
        ...
        loop :while(true){
            ...
            switch (no){
                ...
                default:
                    System.out.println("잘못된 번호를 선택 하셨습니다. 다시 선택 해주세요"); break;
            }
/*
            if(no ==9){         와일 라벨에 루프를 넣고 9번을 입력하면 루프 자체를 종료 시켜버림
                break;
            }*/
        }
    ...
    *
    * 은닉화 : 불필요한 세부 사항을 숨기고 필요한 정보만을 노출한다. 이것은 정보 은닉과 관련이 있으며 객체의 내부 구형을 외부로 부터 숨겨 보호하고
    *         필요한 인터페이스(메소드)만 노출하는 것을 의미한다.
    *
    * 모델링 : 추상화는 현실 세계의 갠며을 소프트웨어 모델로 변환하며 이 모델은 소프트웨어 개발에서 사용된다.
    *         복잡한 현실 세계를 이해하고 해결책을 제공하는 도구로 사용된다.
    *
    * 객체지향 프로그래밍 : 추상화는 객체지향 프로그래밍의 핵심 개념 중 하나로 클래스와 객체를 사용하여 추상화를 통해
    *                   현실 세계의 개념을 모델링하고 이를 소프트웨어로 구현한다. 클래스는 추상 데이터 타입을 정의하고,
    *                   객체는 클래스의 인스턴스로 실제 데이터와 메소드를 갖는다.
    *
    * 인터페이스 ; 인터페이스는 추상화의 한 형태로 객체가 특정 동작을 수행하는 방법을 정의하는 역할을 하며 인터페이스는 추상 메서드를 포함하여
    *            클래스에서 이를 구현함으로서 필요한 동작을 정의할 수 있다.
    * */
}
