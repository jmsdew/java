...
public class Application01 {
    public static void main(String[] args){
        ...
        *    상속 구조가 복잡해 질 수록 그 영향에 대한 예측이 힘들어 이러한 단점이 유지보수성이 증가된다는 장점과는 반대로
        *    유지보수에 악영향을 미친다.
        * 2. 또한 부모클래스의 변경 또한 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능인 경우
        *    부모 클래스를 변경할 시 자식 클래스에 모두 영향을 줄 수 있다.
        *    역시 유지보수성에 악 영향을 미친다.
        * 3. 부모클래스에서는 의미 있었던 가능이 자식 클래스에서는 무의미 할 수 있다.(불필요한 기능이 추가됨)
        *
        * 장점과 단점을 고려했을때
        * 상속은 재 사용이라는 장점만 바라보게 되면 오용 가능성이 있기 때문에 유지 보수에 좋지 않은 코드를 작성할 확률이 높다
        ...
        * */
        ...
        fireCar.toString();  // car의 자손이면서 object의 후손이기때문에 오브젝트 요소를 쓸 수 있음.
        fireCar.run();
        fireCar.soundHorn();
        fireCar.sprayWater();

        Car test = fireCar; // 들어갈 순 있으나 car에 없는 spraywater 메소드는 생략하고 들어간다.
...
//        Firecar firecar1 = racingCar();   자식끼리는 넣어줄 수 없음
    }
}
